...
namespace crow
{
    ...
    namespace black_magic
    {
        ...
        public:
            template <unsigned N> constexpr const_str(const char (&arr)[N]) : begin_(arr), size_(N - 1)
            {
...
#undef CROW_INTERNAL_PARAMETER_TAG
        ...
        template <typename T, typename Tup> typename std::enable_if<!has_type<T&, Tup>::value, T>::type tuple_extract(Tup&)
        {
            return T{};
        }
        ...
        {
            template <template <typename... Args> class U> using rebind =
                typename pop_back_helper<typename gen_seq<sizeof...(T) - 1>::type, std::tuple<T...>>::template rebind<U>;
        };
...
#undef CROW_INTERNAL_PROMOTE_TYPE
    ...
    namespace utility
    {
        ...
        inline static std::string
        base64encode(std::string data, size_t size,
                     const char* key = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/")
        {
        ...
        inline static std::string base64decode(const char* data, size_t size)
        {
            ...
            // Padded
                ...
                size =
                    (size / 4 * 3) - 2; // == padding means the last block only has 1 character instead of 3, hence the '-2'
            else if (size >= 1 && data[size - 1] == '=') // padded with '='
                size =
                    (size / 4 * 3) - 1; // = padding means the last block only has 2 character instead of 3, hence the '-1'
        ...
        inline static void sanitize_filename(std::string& data, char replacement = '_')
        {
            ...
            auto sanitizeSpecialFile =
                [](std::string& source, unsigned ofs, const char* pattern, bool includeNumber, char replacement_)
            {
                unsigned i = ofs;
            ...
            for (unsigned i = 0; i < data.length(); ++i)
            {
                ...
                {
                    if (CROW_UNLIKELY(
                            i == 0)) // Prevent Unix Absolute Paths (Windows Absolute Paths are prevented with `(c == ':')`)
                    {
                        data[i] = replacement;
                    }
                    else
        ...
        template <typename T> inline static T lexical_cast(const char* v, size_t count)
        {
            std::stringstream stream;
            T res;
